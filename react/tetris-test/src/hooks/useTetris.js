...
export default function useTetris() {
  ...
  const movePiece = useCallback(
    (dx, dy) => {
      ...
      if (!checkCollision(board, activePiece.shape, newPos)) {
        ...
        setPosition({ x: 3, y: -1 }); // ← 修正箇所
        setCanHold(true);
        if (checkCollision(clearedBoard, next.shape, { x: 3, y: -1 })) {
          setGameOver(true);
        }
      }
    },
  ...
  const hardDrop = useCallback(() => {
    ...
    let y = position.y;
    while (
      !checkCollision(board, activePiece.shape, {
        x: position.x,
        y: y + 1,
      })
    ) {
  ...
  const ghost = activePiece
    ? getGhost(board, activePiece.shape, position)
    : null;
  ...
  return {
    board: getDisplayBoard(board, activePiece, position),
    activePiece,
    position,
    ghost,
    nextQueue,
